...
namespace GlamlyBackweb.Providers
{
    public class ApplicationOAuthProvider : OAuthAuthorizationServerProvider
    {
        ...
        private IUserServices _userService = new UserServices();
         string hashCode = "", encodingPasswordString = ""; 
        int userid;
        /// <summary>
          ...
          //  string facebookid = form.Result.Get("facebookid");
            ...
            if (context.UserName != null)
            {
               ...
               {                  
                    ...
                    usercollection = javaserializer.Deserialize<UserData>(!string.IsNullOrWhiteSpace(desearlize?.ToString()) ? Convert.ToString(desearlize).Replace("\";", "").Replace("];", "]") : usermetadata.meta_value);
                    ...
                    if(password.Trim() == context.Password.Trim())
                    {
                        user = _userService.validationUserEmail(context.UserName);
                    }
                }
                //if (userdetail != null)
                //{ 
            ...
            if (user.ID > 0)
            {
                ...
                {
                    {
                        "Id", !string.IsNullOrEmpty(user.ID.ToString()) ? user.ID.ToString() : "null"
                    },                  
                    {
