app.controller('BookingController', ['$scope', '$http', 'authData', '$location', '$filter', '$timeout', 'ModelDialogService', function ($scope, $http, authData, $location, $filter, $timeout, modelDialogService) {
    ...
    function DeleteBooking(booking) {
        ...
        .success(function (data) {
            $('#processing').hide();
            if (data.responseCode == 1) {
                modelDialogService.info("Bokningen har tagits bort");
                getbookings();
                //$scope.reset();
            }
            else {
    ...
    $scope.ApproveBooking = function (booking) {
        ...
        if (selectedstylist == undefined) {
            modelDialogService.warning("Vänligen välj stylisten innan du klickar på knappen!");
            $('#processing').hide();
            return;
        }
        $http({
        ...
        .success(function (data) {
            $('#processing').hide();
            if (data.responseCode == 1) {
                modelDialogService.info("Bokningen godkänns av admin framgångsrikt");              
                getbookings();                
            }
            else {
                modelDialogService.info("Internal server error");
