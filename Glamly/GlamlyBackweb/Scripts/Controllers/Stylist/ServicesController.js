app.controller('ServicesController', ['$scope', '$http', 'authData', '$location', '$filter', '$timeout', '$window','$rootScope','ModelDialogService', function ($scope, $http, authData, $location, $filter, $timeout,$window,$rootScope, modelDialogService) {
    ...
    $scope.saveType = function (sname,id) {
        ...
        .success(function (data) {
            ...
            if (data.responseCode == 1) {
                modelDialogService.info("Servicetypen har sparats framgångsrikt");            
              //  $scope.ServicesForm.AddNameText.$setUntouched();
                getAllServices();
                $scope.reset();
            } else if (data.responseCode == 6) {
                modelDialogService.info("Typ av tjänst finns redan");            
            }
            else {
    ...
    $scope.addService = function (sname) {
        ...
        .success(function (data) {
            ...
            if (data.responseCode == 1) {
                modelDialogService.info("Tjänsten har sparats framgångsrikt");
                //  $scope.ServicesForm.AddNameText.$setUntouched();
                getAllServices();
                $scope.reset();
            } else if (data.responseCode == 6) {
                modelDialogService.info("Tjänsten finns redan");
            }
            else {
                modelDialogService.info("Internal server error");
            }
    ...
    $scope.updateService = function (idx) {
        ...
        .success(function (data) {
            $('#processing').hide();
            if (data.responseCode == 1) {
                modelDialogService.info("Servicetypen har uppdaterats framgångsrikt");              
                getAllServices();
                $scope.reset();
            } else if (data == 2) {
                modelDialogService.info("Typ av tjänst finns redan");              
    ...
    function deleteService(Service) {
        ...
        .success(function (data) {
            $('#processing').hide();
            if (data.responseCode == 1) {
                modelDialogService.info("Servicetypen har tagits bort");           
                getAllServices();
                $scope.reset();
            }
            else {
